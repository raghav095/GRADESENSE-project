...
- **A photographed handwritten answer sheet is a real, supported case, not just PDFs of typed text.** Upload a PNG/JPG (or a scanned PDF with no text layer) and, with a live Gemini key configured, GradeSense reads it via AI vision transcription instead of just failing — including a hand-drawn diagram on that same page, which gets genuinely assessed against its rubric criterion rather than just flagged blind. This is always flagged for human review when it happens — AI transcription of handwriting isn't guaranteed accurate — and if no live credentials are configured or the attempt fails, the result is flagged as genuinely uncertain (not scored as a confident zero, which would be silently wrong). The exported annotated PDF also embeds the original photo itself as its own page, so a teacher can check every mark against the real handwriting without the app open.
